...
async function runGold1Card1WeekSequentialAudit() {
  console.log("================================================================================");
  console.log("📊 REAL SINGLE-CARD 1-WEEK SEQUENTIAL LIFECYCLE SIMULATION (XAUTUSDT / GOLD)");
  console.log("================================================================================");
  ...
  while (currentStart < endTime) {
    try {
      ...
      const klines = await mexcClient.getKlines('GOLD(XAUT)USDT', '1m', 1000, currentStart, nextEnd);
  ...
  console.log(`Loaded ${allKlines.length} 1-minute candles for XAUTUSDT across 7 days.`);
  ...
  for (let i = 0; i < allKlines.length; i++) {
    ...
    if (cardState === 'SCANNING') {
      ...
      if (baseObi >= 70.0 && minFloor >= 55.0) {
        // TRIGGER ENTRY FOR SINGLE CARD (XAUTUSDT)!
  ...
  console.log("\n================================================================================");
  console.log(`🏆 XAUTUSDT (GOLD) SINGLE-CARD 1-WEEK SIMULATION RESULTS:`);
  ...
  let markdown = `# 🛡️ Single Trading Card 1-Week Sequential Lifecycle Report (GOLD / XAUTUSDT)
...
**Target Pair**: \`XAUTUSDT\` (Tether Gold)  
**Rules Enforced**:
...
## 🏆 1-Card 1-Week Sequential Summary for GOLD (XAUTUSDT)

- **Total Trades Taken by 1 Card**: **${completedTrades.length} Trades**
- **Completed Take Profit (+0.60%) Wins**: **${wins.length} Trades (${((wins.length / (completedTrades.length || 1)) * 100).toFixed(1)}% Win Rate)** 🟢
...
## 📅 Step-by-Step Timeline of 1 Card Lifecycle for GOLD (XAUTUSDT) (Full Week)
...
`;
  ...
  completedTrades.forEach(t => {
    if (t.status === 'TP_HIT') {
      markdown += `| **Trade #${t.tradeNo}** | \`${t.entryTimeStr}\` | **$${t.buyPrice.toFixed(2)}** | **${t.obiPct}%** | **$${t.tpPrice}** | \`${t.exitTimeStr}\` | **${t.durationMins} Mins** (${(t.durationMins / 60).toFixed(1)}h) | ✅ +0.60% TP FILLED |\n`;
    } else {
      markdown += `| **Trade #${t.tradeNo}** | \`${t.entryTimeStr}\` | **$${t.buyPrice.toFixed(2)}** | **${t.obiPct}%** | **$${t.tpPrice}** | *Holding...* | *Active...* | ⏳ PENDING SAFE HOLD |\n`;
    }
  });

  const artifactPath = 'C:\\Users\\Hi\\.gemini\\antigravity\\brain\\cdfb16e8-d8e7-4868-967f-4d9834b72016\\gold_xaut_single_card_1week_sequential_lifecycle_report.md';
  fs.writeFileSync(artifactPath, markdown);
  console.log(`\n✅ Artifact written to: ${artifactPath}`);
}

runGold1Card1WeekSequentialAudit().catch(console.error);
