...
function getSyntheticObi(candle, prevCandle) {
  ...
  const closePosition = (close - low) / range; // 0.0 (low) to 1.0 (high)
...
async function runGoldRsiAudit() {
  console.log("================================================================================");
  console.log("🥇 GOLD (PAXGUSDT / XAUTUSDT) 1-YEAR OBI >= 55.0% & RSI < 45 vs RSI < 50 AUDIT");
  console.log("================================================================================");
  ...
  let countObi55Rsi45 = 0;
  let countObi55Rsi50 = 0;
  ...
  for (let i = 16; i < candles.length; i++) {
    ...
    const openTimeStr = new Date(timestampMs + 5 * 3600 * 1000 + 30 * 60 * 1000).toISOString().replace('T', ' ').substring(0, 16) + ' PKT';
    const closePrice = parseFloat(candle[4]);

    const rsiVal = rsi4h[i];
    const obiVal = getSyntheticObi(candle, candles[i - 1]);

    const isObi55 = (obiVal >= 55.0);
    ...
    if (isObi55 && isRsi45) {
      ...
      if (i - lastSignal45Index >= 16) { // 4-hour cooldown between distinct trade entries
        distinctTrades45++;
        lastSignal45Index = i;
      }
    }
  ...
  console.log("\n================================================================================");
  console.log("📊 GOLD (PAXG) 1-YEAR AUDIT RESULTS SUMMARY");
  console.log("================================================================================");
  console.log(`1️⃣ CONDITION A: OBI >= 55.0% AND 4h 15m RSI < 45.0`);
  console.log(`   - Total 15m Candles/Scans Matched: ${countObi55Rsi45} Candles`);
  console.log(`   - Distinct Trade Signal Entries Triggered (4h Cooldown): ${distinctTrades45} Trades`);

  console.log(`\n2️⃣ CONDITION B: OBI >= 55.0% AND 4h 15m RSI < 50.0`);
  console.log(`   - Total 15m Candles/Scans Matched: ${countObi55Rsi50} Candles`);
  console.log(`   - Distinct Trade Signal Entries Triggered (4h Cooldown): ${distinctTrades50} Trades`);

  console.log(`\n3️⃣ COMPARISON & DIFFERENCE:`);
  console.log(`   - RSI < 50 filter captures +${countObi55Rsi50 - countObi55Rsi45} more candles (+${(((countObi55Rsi50 - countObi55Rsi45) / countObi55Rsi45) * 100).toFixed(1)}%) than RSI < 45.`);
  console.log(`   - Distinct Trade Entries increase from ${distinctTrades45} Trades (RSI < 45) to ${distinctTrades50} Trades (RSI < 50).`);

  console.log("\n--------------------------------------------------------------------------------");
  console.log("📅 SAMPLE RECENT MATCHED TIMESTAMPS FOR GOLD (RSI < 45 & OBI >= 55%):");
  matchesRsi45.slice(-5).forEach((m, idx) => {
    console.log(`   [#${idx+1}] ${m.time} | Gold Price: $${m.price.toFixed(2)} USDT | OBI: ${m.obi.toFixed(1)}% | 4h 15m RSI: ${m.rsi.toFixed(1)}`);
  });

  console.log("\n--------------------------------------------------------------------------------");
  console.log("📅 SAMPLE RECENT MATCHED TIMESTAMPS FOR GOLD (RSI < 50 & OBI >= 55%):");
  ...
  const reportData = {
    ...
    conditionA_Rsi45: {
      totalCandlesMatched: countObi55Rsi45,
      distinctTrades: distinctTrades45,
      samples: matchesRsi45.slice(-10)
    },
    conditionB_Rsi50: {
      totalCandlesMatched: countObi55Rsi50,
  ...
  fs.writeFileSync(
    path.join(__dirname, 'gold_rsi45_vs_rsi50_obi55_audit_report.json'),
    JSON.stringify(reportData, null, 2)
  );

  console.log("\n✅ Saved complete Gold audit report to backend/gold_rsi45_vs_rsi50_obi55_audit_report.json");
}

runGoldRsiAudit().catch(err => {
  console.error("❌ Gold Audit Error:", err);
});
